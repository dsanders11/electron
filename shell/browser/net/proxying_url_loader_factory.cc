...
ProxyingURLLoaderFactory::InProgressRequest::InProgressRequest(
  ...
  target_client_.set_disconnect_handler(base::BindOnce(
      &ProxyingURLLoaderFactory::InProgressRequest::OnClientDisconnected,
      weak_factory_.GetWeakPtr()));
  proxied_loader_receiver_.set_disconnect_handler(base::BindOnce(
      &ProxyingURLLoaderFactory::InProgressRequest::OnNetworkError,
      weak_factory_.GetWeakPtr(),
...
bool ProxyingURLLoaderFactory::IsForDownload() const {
  return false;
}
...
void ProxyingURLLoaderFactory::InProgressRequest::ContinueToSendHeadersWithOk(
    const std::set<std::string>& removed_headers,
    const std::set<std::string>& set_headers) {
...
void ProxyingURLLoaderFactory::InProgressRequest::OnNetworkError(
    const network::URLLoaderCompletionStatus& status) {
  State state = state_;
  if (state_ == State::kInProgress) {
...
void ProxyingURLLoaderFactory::InProgressRequest::OnClientDisconnected() {
  State state = state_;
  if (state_ == State::kInProgress) {
...
bool ProxyingURLLoaderFactory::InProgressRequest::IsRedirectSafe(
    const GURL& from_url,
    const GURL& to_url,
    bool is_navigation_request) {
  // Note: In Electron we allow all redirects.
  return true;
}

network::URLLoaderCompletionStatus
ProxyingURLLoaderFactory::InProgressRequest::CreateURLLoaderCompletionStatus(
    int error_code,
...
void ProxyingURLLoaderFactory::CreateLoaderAndStart(
  ...
  if (IsForServiceWorkerScript() && request.url.SchemeIsFile()) {
    asar::CreateAsarURLLoader(request, std::move(loader_receiver),
                              std::move(client),
  ...
  auto result = requests_.emplace(
      ...
      std::make_unique<InProgressRequest>(
          ...
          options, request, traffic_annotation, std::move(loader_receiver),
          std::move(client)));
