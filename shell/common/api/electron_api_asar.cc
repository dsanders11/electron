...
std::unique_ptr<asar::Archive> GetAsarArchive(const base::FilePath& path) {
  if (path.BaseName().MaybeAsASCII() == "app.asar") {
    ...
    size_t size;
    void* ptr = postject_find_resource("app_asar", &size, &postject_options);

    if (ptr && size > 0) {
      return std::make_unique<asar::Archive>(path, reinterpret_cast<const uint8_t*>(ptr), size);
    }
  }
