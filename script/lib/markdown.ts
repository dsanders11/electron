...
function positionToRange (position: Position): Range {
  return {
    start: { character: position.start.column - 1, line: position.start.line - 1 },
...
export class MarkdownLinkComputer implements IMdLinkComputer {
  ...
  async getAllLinks (document: ITextDocument): Promise<MdLink[]> {
    const { fromMarkdown } = await dynamicImport(
      'mdast-util-from-markdown') as { fromMarkdown: typeof FromMarkdownFunction };
  ...
  async #getInlineLinks (document: ITextDocument, tree: Node): Promise<MdLink[]> {
    const { visit } = await dynamicImport('unist-util-visit') as { visit: typeof VisitFunction };

    const documentUri = URI.parse(document.uri);
    const links: MdLink[] = [];

    visit(tree, node => node.type === 'link', (node: Node) => {
  ...
  async #getLinkDefinitions (document: ITextDocument, tree: Node): Promise<MdLink[]> {
    ...
    visit(tree, node => node.type === 'definition', (node: Node) => {
